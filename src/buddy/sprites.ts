import type { CompanionBones, Eye, Hat, Species } from './types.js'
import type { Emotion } from './emotions.js'
import {
  axolotl,
  blob,
  cactus,
  capybara,
  cat,
  chonk,
  dragon,
  duck,
  ghost,
  goose,
  mushroom,
  octopus,
  owl,
  penguin,
  rabbit,
  robot,
  snail,
  turtle,
} from './types.js'

// Each sprite is 5 lines tall, 12 wide (after {E}→1char substitution).
// Multiple frames per species for idle fidget animation.
// Line 0 is the hat slot — must be blank in frames 0-1; frame 2 may use it.
const BODIES: Record<Species, string[][]> = {
  [duck]: [
    [
      '            ',
      '    __      ',
      '  <({E} )___  ',
      '   (  ._>   ',
      '    `--´    ',
    ],
    [
      '            ',
      '    __      ',
      '  <({E} )___  ',
      '   (  ._>   ',
      '    `--´~   ',
    ],
    [
      '            ',
      '    __      ',
      '  <({E} )___  ',
      '   (  .__>  ',
      '    `--´    ',
    ],
  ],
  [goose]: [
    [
      '            ',
      '     ({E}>    ',
      '     ||     ',
      '   _(__)_   ',
      '    ^^^^    ',
    ],
    [
      '            ',
      '    ({E}>     ',
      '     ||     ',
      '   _(__)_   ',
      '    ^^^^    ',
    ],
    [
      '            ',
      '     ({E}>>   ',
      '     ||     ',
      '   _(__)_   ',
      '    ^^^^    ',
    ],
  ],
  [blob]: [
    [
      '            ',
      '   .----.   ',
      '  ( {E}  {E} )  ',
      '  (      )  ',
      '   `----´   ',
    ],
    [
      '            ',
      '  .------.  ',
      ' (  {E}  {E}  ) ',
      ' (        ) ',
      '  `------´  ',
    ],
    [
      '            ',
      '    .--.    ',
      '   ({E}  {E})   ',
      '   (    )   ',
      '    `--´    ',
    ],
  ],
  [cat]: [
    [
      '            ',
      '   /\\_/\\    ',
      '  ( {E}   {E})  ',
      '  (  ω  )   ',
      '  (")_(")   ',
    ],
    [
      '            ',
      '   /\\_/\\    ',
      '  ( {E}   {E})  ',
      '  (  ω  )   ',
      '  (")_(")~  ',
    ],
    [
      '            ',
      '   /\\-/\\    ',
      '  ( {E}   {E})  ',
      '  (  ω  )   ',
      '  (")_(")   ',
    ],
  ],
  [dragon]: [
    [
      '            ',
      '  /^\\  /^\\  ',
      ' <  {E}  {E}  > ',
      ' (   ~~   ) ',
      '  `-vvvv-´  ',
    ],
    [
      '            ',
      '  /^\\  /^\\  ',
      ' <  {E}  {E}  > ',
      ' (        ) ',
      '  `-vvvv-´  ',
    ],
    [
      '   ~    ~   ',
      '  /^\\  /^\\  ',
      ' <  {E}  {E}  > ',
      ' (   ~~   ) ',
      '  `-vvvv-´  ',
    ],
  ],
  [octopus]: [
    [
      '            ',
      '   .----.   ',
      '  ( {E}  {E} )  ',
      '  (______)  ',
      '  /\\/\\/\\/\\  ',
    ],
    [
      '            ',
      '   .----.   ',
      '  ( {E}  {E} )  ',
      '  (______)  ',
      '  \\/\\/\\/\\/  ',
    ],
    [
      '     o      ',
      '   .----.   ',
      '  ( {E}  {E} )  ',
      '  (______)  ',
      '  /\\/\\/\\/\\  ',
    ],
  ],
  [owl]: [
    [
      '            ',
      '   /\\  /\\   ',
      '  (({E})({E}))  ',
      '  (  ><  )  ',
      '   `----´   ',
    ],
    [
      '            ',
      '   /\\  /\\   ',
      '  (({E})({E}))  ',
      '  (  ><  )  ',
      '   .----.   ',
    ],
    [
      '            ',
      '   /\\  /\\   ',
      '  (({E})(-))  ',
      '  (  ><  )  ',
      '   `----´   ',
    ],
  ],
  [penguin]: [
    [
      '            ',
      '  .---.     ',
      '  ({E}>{E})     ',
      ' /(   )\\    ',
      '  `---´     ',
    ],
    [
      '            ',
      '  .---.     ',
      '  ({E}>{E})     ',
      ' |(   )|    ',
      '  `---´     ',
    ],
    [
      '  .---.     ',
      '  ({E}>{E})     ',
      ' /(   )\\    ',
      '  `---´     ',
      '   ~ ~      ',
    ],
  ],
  [turtle]: [
    [
      '            ',
      '   _,--._   ',
      '  ( {E}  {E} )  ',
      ' /[______]\\ ',
      '  ``    ``  ',
    ],
    [
      '            ',
      '   _,--._   ',
      '  ( {E}  {E} )  ',
      ' /[______]\\ ',
      '   ``  ``   ',
    ],
    [
      '            ',
      '   _,--._   ',
      '  ( {E}  {E} )  ',
      ' /[======]\\ ',
      '  ``    ``  ',
    ],
  ],
  [snail]: [
    [
      '            ',
      ' {E}    .--.  ',
      '  \\  ( @ )  ',
      '   \\_`--´   ',
      '  ~~~~~~~   ',
    ],
    [
      '            ',
      '  {E}   .--.  ',
      '  |  ( @ )  ',
      '   \\_`--´   ',
      '  ~~~~~~~   ',
    ],
    [
      '            ',
      ' {E}    .--.  ',
      '  \\  ( @  ) ',
      '   \\_`--´   ',
      '   ~~~~~~   ',
    ],
  ],
  [ghost]: [
    [
      '            ',
      '   .----.   ',
      '  / {E}  {E} \\  ',
      '  |      |  ',
      '  ~`~``~`~  ',
    ],
    [
      '            ',
      '   .----.   ',
      '  / {E}  {E} \\  ',
      '  |      |  ',
      '  `~`~~`~`  ',
    ],
    [
      '    ~  ~    ',
      '   .----.   ',
      '  / {E}  {E} \\  ',
      '  |      |  ',
      '  ~~`~~`~~  ',
    ],
  ],
  [axolotl]: [
    [
      '            ',
      '}~(______)~{',
      '}~({E} .. {E})~{',
      '  ( .--. )  ',
      '  (_/  \\_)  ',
    ],
    [
      '            ',
      '~}(______){~',
      '~}({E} .. {E}){~',
      '  ( .--. )  ',
      '  (_/  \\_)  ',
    ],
    [
      '            ',
      '}~(______)~{',
      '}~({E} .. {E})~{',
      '  (  --  )  ',
      '  ~_/  \\_~  ',
    ],
  ],
  [capybara]: [
    [
      '            ',
      '  n______n  ',
      ' ( {E}    {E} ) ',
      ' (   oo   ) ',
      '  `------´  ',
    ],
    [
      '            ',
      '  n______n  ',
      ' ( {E}    {E} ) ',
      ' (   Oo   ) ',
      '  `------´  ',
    ],
    [
      '    ~  ~    ',
      '  u______n  ',
      ' ( {E}    {E} ) ',
      ' (   oo   ) ',
      '  `------´  ',
    ],
  ],
  [cactus]: [
    [
      '            ',
      ' n  ____  n ',
      ' | |{E}  {E}| | ',
      ' |_|    |_| ',
      '   |    |   ',
    ],
    [
      '            ',
      '    ____    ',
      ' n |{E}  {E}| n ',
      ' |_|    |_| ',
      '   |    |   ',
    ],
    [
      ' n        n ',
      ' |  ____  | ',
      ' | |{E}  {E}| | ',
      ' |_|    |_| ',
      '   |    |   ',
    ],
  ],
  [robot]: [
    [
      '            ',
      '   .[||].   ',
      '  [ {E}  {E} ]  ',
      '  [ ==== ]  ',
      '  `------´  ',
    ],
    [
      '            ',
      '   .[||].   ',
      '  [ {E}  {E} ]  ',
      '  [ -==- ]  ',
      '  `------´  ',
    ],
    [
      '     *      ',
      '   .[||].   ',
      '  [ {E}  {E} ]  ',
      '  [ ==== ]  ',
      '  `------´  ',
    ],
  ],
  [rabbit]: [
    [
      '            ',
      '   (\\__/)   ',
      '  ( {E}  {E} )  ',
      ' =(  ..  )= ',
      '  (")__(")  ',
    ],
    [
      '            ',
      '   (|__/)   ',
      '  ( {E}  {E} )  ',
      ' =(  ..  )= ',
      '  (")__(")  ',
    ],
    [
      '            ',
      '   (\\__/)   ',
      '  ( {E}  {E} )  ',
      ' =( .  . )= ',
      '  (")__(")  ',
    ],
  ],
  [mushroom]: [
    [
      '            ',
      ' .-o-OO-o-. ',
      '(__________)',
      '   |{E}  {E}|   ',
      '   |____|   ',
    ],
    [
      '            ',
      ' .-O-oo-O-. ',
      '(__________)',
      '   |{E}  {E}|   ',
      '   |____|   ',
    ],
    [
      '   . o  .   ',
      ' .-o-OO-o-. ',
      '(__________)',
      '   |{E}  {E}|   ',
      '   |____|   ',
    ],
  ],
  [chonk]: [
    [
      '            ',
      '  /\\    /\\  ',
      ' ( {E}    {E} ) ',
      ' (   ..   ) ',
      '  `------´  ',
    ],
    [
      '            ',
      '  /\\    /|  ',
      ' ( {E}    {E} ) ',
      ' (   ..   ) ',
      '  `------´  ',
    ],
    [
      '            ',
      '  /\\    /\\  ',
      ' ( {E}    {E} ) ',
      ' (   ..   ) ',
      '  `------´~ ',
    ],
  ],
}

// EMOTION_BODIES[species][emotion] = [frame0, frame1, frame2]
// Each frame is 5 lines, line 0 is hat slot (keep blank unless species uses it)
// {E} is the eye slot placeholder (renderSprite replaces it)
const EMOTION_BODIES: Record<Species, Record<Emotion, string[][]>> = {
  [duck]: {
    happy: [
      ['            ', '   \\^^^/    ', '    __      ', '  <({E}~)___  ', "    `--´~   "],
      ['            ', '    __      ', '  <({E}~)___  ', '   (  ._>~  ', "    `--´    "],
      ['            ', '   \\^^^/    ', '    __      ', '  <({E}~)___  ', "    `--´~   "],
    ],
    proud: [
      ['            ', '     __    ', '   <({E})___ ', '    (  ._>  ', "    /´--    "],
      ['            ', '    __     ', '   <({E})___ ', '    (  ._>  ', "    /´--    "],
      ['            ', '     __    ', '   <({E})___ ', '    (  ._>  ', "    /´--~   "],
    ],
    curious: [
      ['            ', '    __/    ', '  <({E} )___  ', '   (  ._>   ', "    `--´    "],
      ['            ', '    __/    ', '  <({E} )___  ', '   (  ._>   ', "    `--´    "],
      ['            ', '    __/    ', '  <({E}~)___  ', '   (  ._>   ', "    `--´    "],
    ],
    pensive: [
      ['            ', '    __     ', '  <({E} )___  ', '   (  ·_>   ', "    `--´    "],
      ['            ', '   __      ', '  <({E} )___  ', '   (  ·_>   ', "    `--´    "],
      ['            ', '    __     ', '  <({E} )___  ', '   (  ·_>   ', "    `--´    "],
    ],
    tired: [
      ['            ', '    __    ', '  <(· )___  ', '   (  ·_>   ', "    `--´    "],
      ['            ', '   __     ', '  <(· )___  ', '   (  ·_>   ', "    `--´    "],
      ['            ', '    __    ', '  <(· )___  ', '   (  ·_>   ', "    `--´    "],
    ],
    snarky: [
      ['            ', '    __     ', '  <({E}~)___  ', '   (  ·_>~  ', "    /´~    "],
      ['            ', '    __     ', '  <({E}~)___  ', '   (  ·_>   ', "    /´    "],
      ['            ', '    __     ', '  <({E}~)___  ', '   (  ·_>~  ', "    /´~    "],
    ],
  },
  [goose]: {
    happy: [
      ['            ', '    ({E}>    ', '    || ~   ', '  _(__)_   ', '   ^^^^~   '],
      ['            ', '    ({E}>    ', '    || ~   ', '  _(__)_   ', '   ^^^^~   '],
      ['            ', '    ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
    ],
    proud: [
      ['            ', '     ({E}>>  ', '     ||    ', '   _(__)_  ', '    ^^^^   '],
      ['            ', '    ({E}>>   ', '     ||    ', '   _(__)_  ', '    ^^^^   '],
      ['            ', '     ({E}>>  ', '     ||    ', '   _(__)_  ', '    ^^^^~  '],
    ],
    curious: [
      ['            ', '    ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '   /({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '    ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
    ],
    pensive: [
      ['            ', '    ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '   ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '    ({E}>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
    ],
    tired: [
      ['            ', '   (·)>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '  (·)>     ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
      ['            ', '   (·)>    ', '    ||     ', '  _(__)_   ', '   ^^^^    '],
    ],
    snarky: [
      ['            ', '    ({E}>    ', '    || ~   ', '  _(__)_   ', '   ^~^^~   '],
      ['            ', '    ({E}>    ', '   ~|| ~   ', '  _(__)_   ', '   ^~^^~   '],
      ['            ', '    ({E}>    ', '    || ~   ', '  _(__)_   ', '   ^~^^~   '],
    ],
  },
  [blob]: {
    happy: [
      ['            ', '   .----.  ~', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
      ['            ', '   .----.  ~', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
    ],
    proud: [
      ['            ', '  .------.  ', ' (  {E}  {E}  ) ', ' (        ) ', "  `------´  "],
      ['            ', ' .--------. ', ' (  {E}  {E}  ) ', ' (        ) ', "  `--------´ "],
      ['            ', '  .------.  ', ' (  {E}  {E}  ) ', ' (        ) ', "  `------´  "],
    ],
    curious: [
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (      )  ', "   `----´   "],
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (      )  ', "   `----´   "],
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (      )  ', "   `----´   "],
    ],
    pensive: [
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
      ['            ', '  .------.  ', ' (  {E}  {E}  ) ', ' (        ) ', "  `------´  "],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
    ],
    tired: [
      ['            ', '  .------.  ', ' (  ·  ·  ) ', ' (        ) ', "  `------´  "],
      ['            ', ' .--------. ', ' (  ·  ·  ) ', ' (        ) ', "  `--------´ "],
      ['            ', '  .------.  ', ' (  ·  ·  ) ', ' (        ) ', "  `------´  "],
    ],
    snarky: [
      ['            ', '   .----. ~ ', '  ( {E}  {E} )  ', '  (      )~ ', "   `----´   "],
      ['            ', '   .----. ~ ', '  ( {E}  {E} )  ', '  (      )  ', "   `----´   "],
      ['            ', '   .----. ~ ', '  ( {E}  {E} )  ', '  (      )~ ', "   `----´   "],
    ],
  },
  [cat]: {
    happy: [
      ['            ', '   /\\_/\\  ~ ', '  ( {E}   {E})  ', '  (  ω  )  ~', '  (")_(")  ~'],
      ['            ', '   /\\_/\\  ~ ', '  ( {E}   {E})  ', '  (  ω  )~  ', '  (")_(")~  '],
      ['            ', '   /\\_/\\    ', '  ( {E}   {E})  ', '  (  ω  )   ', '  (")_(")   '],
    ],
    proud: [
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['            ', ' /^^^\\^^^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
    ],
    curious: [
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})/ ', '  (  ω  )   ', '  (")_(")   '],
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})/ ', '  (  ω  )   ', '  (")_(")   '],
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})/ ', '  (  ω  )   ', '  (")_(")   '],
    ],
    pensive: [
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})  ', '  (  ω  )   ', '  (")_(")   '],
      ['            ', '  /\\- /\\   ', '  ( {E}   {E})  ', '  (  ω  )   ', '  (")_(")   '],
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})  ', '  (  ω  )   ', '  (")_(")   '],
    ],
    tired: [
      ['            ', '   /\\_/\\   ', '  ( ·   · )  ', '  (  ·  )   ', '  (")_(")   '],
      ['            ', '   /\\_/\\   ', '  ( {E}   {E})  ', '  (  ·  )   ', '  (")_(")~  '],
      ['            ', '   /\\_/\\   ', '  ( ·   · )  ', '  (  ·  )   ', '  (")_(")   '],
    ],
    snarky: [
      ['            ', '   /\\_/\\ ~ ', '  ( {E}   {E})~ ', '  (  ~  )~  ', '  (")_(")~  '],
      ['            ', '   /\\_/\\ ~ ', '  ( {E}   {E})~ ', '  (  ~  )   ', '  (")_(")~  '],
      ['            ', '   /\\_/\\ ~ ', '  ( {E}   {E})~ ', '  (  ~  )~  ', '  (")_(")~  '],
    ],
  },
  [dragon]: {
    happy: [
      ['            ', '  /^\\  /^\\~ ', ' <  {E}  {E}  > ', ' (   ~~   )~', '  `-vvvv-´~ '],
      ['            ', '  /^\\  /^\\~ ', ' <  {E}  {E}  > ', ' (   ~~   )~', '  `-vvvv-´~ '],
      ['            ', '  /^\\  /^\\  ', ' <  {E}  {E}  > ', ' (   ~~   ) ', '  `-vvvv-´  '],
    ],
    proud: [
      ['   ~    ~   ', '  /^\\  /^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['  ~      ~  ', '  /^\\  /^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['   ~    ~   ', '  /^\\  /^\\ ', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
    ],
    curious: [
      ['            ', '  /^\\  /^\\/', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['            ', '  /^\\  /^\\/', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
      ['            ', '  /^\\  /^\\/', ' <  {E}  {E}  >', ' (   ~~   )', '  `-vvvv-´ '],
    ],
    pensive: [
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  > ', ' (   ~~   ) ', '  `-vvvv-´  '],
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  > ', ' (   ..   ) ', '  `-vvvv-´  '],
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  > ', ' (   ~~   ) ', '  `-vvvv-´  '],
    ],
    tired: [
      ['            ', '  /^\\  /^\\ ', ' <  ·  ·  > ', ' (   ..   ) ', '  `-vvvv-´  '],
      ['            ', '  /^\\  /^\\ ', ' <  {E}  {E}  > ', ' (   ..   ) ', '  `-vvvv-´~ '],
      ['            ', '  /^\\  /^\\ ', ' <  ·  ·  > ', ' (   ..   ) ', '  `-vvvv-´  '],
    ],
    snarky: [
      ['            ', '  /^\\  /^\\~', ' <  {E}  {E}  >~', ' (   ~~   )~', '  `-vvvv-´~ '],
      ['            ', '  /^\\  /^\\~', ' <  {E}  {E}  >~', ' (   ~~   ) ', '  `-vvvv-´~ '],
      ['            ', '  /^\\  /^\\~', ' <  {E}  {E}  >~', ' (   ~~   )~', '  `-vvvv-´~ '],
    ],
  },
  [octopus]: {
    happy: [
      ['            ', '   .----. ~ ', '  ( {E}  {E} )~ ', '  (______)~ ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----. ~ ', '  ( {E}  {E} )~ ', '  (______)~ ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
    ],
    proud: [
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
      ['            ', '  .------.  ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
    ],
    curious: [
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (______)  ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (______)  ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )/ ', '  (______)  ', '  /\\/\\/\\/\\  '],
    ],
    pensive: [
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
      ['            ', '   .----.   ', '  ( ·  · )  ', '  (______)  ', '  \\/\\/\\/\\/  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\  '],
    ],
    tired: [
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  \\/\\/\\/\\/  '],
      ['            ', '  .------.  ', '  ( ·  · )  ', '  (______)  ', '  \\/\\/\\/\\/  '],
      ['            ', '   .----.   ', '  ( {E}  {E} )  ', '  (______)  ', '  \\/\\/\\/\\/  '],
    ],
    snarky: [
      ['            ', '   .----. ~ ', '  ( {E}  {E} )~ ', '  (______)  ', '  /\\/\\/\\/\\~ '],
      ['            ', '   .----. ~ ', '  ( {E}  {E} )  ', '  (______)  ', '  /\\/\\/\\/\\~ '],
      ['            ', '   .----. ~ ', '  ( {E}  {E} )~ ', '  (______)  ', '  /\\/\\/\\/\\~ '],
    ],
  },
  [owl]: {
    happy: [
      ['            ', '   /\\  /\\~ ', '  (({E})({E}))~ ', '  (  ><  )~ ', "   `----´~  "],
      ['            ', '   /\\  /\\~ ', '  (({E})({E}))~ ', '  (  ><  )~ ', "   `----´~  "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "   `----´   "],
    ],
    proud: [
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "  .----.   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "   `----´   "],
    ],
    curious: [
      ['            ', '   /\\  /\\  ', '  (({E})({E}))/ ', '  (  ><  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))/ ', '  (  ><  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))/ ', '  (  ><  )  ', "   `----´   "],
    ],
    pensive: [
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})(-))  ', '  (  ><  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ><  )  ', "   `----´   "],
    ],
    tired: [
      ['            ', '   /\\  /\\  ', '  ( (·)(·) )  ', '  (  ··  )  ', "   `----´   "],
      ['            ', '   /\\  /\\  ', '  (({E})({E}))  ', '  (  ··  )  ', "   .----.   "],
      ['            ', '   /\\  /\\  ', '  ( (·)(·) )  ', '  (  ··  )  ', "   `----´   "],
    ],
    snarky: [
      ['            ', '   /\\  /\\~ ', '  (({E})({E}))~ ', '  (  ><  )~ ', "   `----´~  "],
      ['            ', '   /\\  /\\~ ', '  (({E})({E}))~ ', '  (  ><  )  ', "   `----´~  "],
      ['            ', '   /\\  /\\~ ', '  (({E})({E}))~ ', '  (  ><  )~ ', "   `----´~  "],
    ],
  },
  [penguin]: {
    happy: [
      ['            ', '  .---.     ', '  ({E}>{E})~    ', ' /(   )\\~   ', "   `---´~   "],
      ['            ', '  .---.     ', '  ({E}>{E})~    ', ' /(   )\\~   ', "   `---´~   "],
      ['            ', '  .---.     ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
    ],
    proud: [
      ['            ', '  .---.     ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
      ['            ', '  .---.     ', '  ({E}>{E})     ', ' |(   )|    ', "   `---´    "],
      ['            ', '  .---.     ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
    ],
    curious: [
      ['            ', '  .---./   ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
      ['            ', '  .---./   ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
      ['            ', '  .---./   ', '  ({E}>{E})     ', ' /(   )\\    ', "   `---´    "],
    ],
    pensive: [
      ['            ', '  .---.    ', '  ({E}>{E})     ', ' /(   )\\   ', "   `---´   "],
      ['            ', '  .---.    ', '  ({E}>{E})     ', ' |(   )|   ', "   `---´   "],
      ['            ', '  .---.    ', '  ({E}>{E})     ', ' /(   )\\   ', "   `---´   "],
    ],
    tired: [
      ['            ', '  .---.    ', '  (·>·)     ', ' |(   )|   ', "   `---´   "],
      ['            ', '  .---.    ', '  ({E}>{E})     ', ' |(   )|   ', "   `---´~  "],
      ['            ', '  .---.    ', '  (·>·)     ', ' |(   )|   ', "   `---´   "],
    ],
    snarky: [
      ['            ', '  .---.    ', '  ({E}>{E})~    ', ' /(   )\\~  ', "   `---´~  "],
      ['            ', '  .---.    ', '  ({E}>{E})~    ', ' /(   )\\   ', "   `---´~  "],
      ['            ', '  .---.    ', '  ({E}>{E})~    ', ' /(   )\\~  ', "   `---´~  "],
    ],
  },
  [turtle]: {
    happy: [
      ['            ', '   _,--._  ~', '  ( {E}  {E} )~ ', ' /[______]\\~', '  ``    ``  '],
      ['            ', '   _,--._  ~', '  ( {E}  {E} )~ ', ' /[______]\\~', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``  '],
    ],
    proud: [
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[======]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``  '],
    ],
    curious: [
      ['            ', '   _,--._  ', '  ( {E}  {E} )/ ', ' /[______]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )/ ', ' /[______]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )/ ', ' /[______]\\ ', '  ``    ``  '],
    ],
    pensive: [
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '   ``  ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``  '],
    ],
    tired: [
      ['            ', '   _,--._  ', '  ( ·  · )  ', ' /[______]\\ ', '  ``    ``  '],
      ['            ', '   _,--._  ', '  ( {E}  {E} )  ', ' /[______]\\ ', '  ``    ``~ '],
      ['            ', '   _,--._  ', '  ( ·  · )  ', ' /[______]\\ ', '  ``    ``  '],
    ],
    snarky: [
      ['            ', '   _,--._~ ', '  ( {E}  {E} )~ ', ' /[______]\\~', '  ``    ``~ '],
      ['            ', '   _,--._~ ', '  ( {E}  {E} )~ ', ' /[______]\\ ', '  ``    ``~ '],
      ['            ', '   _,--._~ ', '  ( {E}  {E} )~ ', ' /[______]\\~', '  ``    ``~ '],
    ],
  },
  [snail]: {
    happy: [
      ['            ', ' {E}   .--.~ ', '  \\ ( @ )~  ', '   \\_`--´~  ', '  ~~~~~~~~  '],
      ['            ', ' {E}   .--.  ', '  \\ ( @ )~  ', '   \\_`--´~  ', '  ~~~~~~~~~ '],
      ['            ', ' {E}   .--.~ ', '  \\ ( @ )~  ', '   \\_`--´~  ', '  ~~~~~~~~  '],
    ],
    proud: [
      ['            ', ' {E}   .--.  ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
      ['            ', ' {E}  .--.   ', '   \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
      ['            ', ' {E}   .--.  ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
    ],
    curious: [
      ['            ', ' {E}   .--./ ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
      ['            ', ' {E}   .--./ ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
      ['            ', ' {E}   .--./ ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
    ],
    pensive: [
      ['            ', ' {E}   .--.  ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
      ['            ', ' {E}   .--.  ', '  \\ ( @ )  ', '   \\_`--´  ', '   ~~~~~~  '],
      ['            ', ' {E}   .--.  ', '  \\ ( @ )  ', '   \\_`--´  ', '  ~~~~~~~  '],
    ],
    tired: [
      ['            ', ' {E}   .--.  ', '   ( · )  ', '   ~~~~~~  ', '   ~~~~~~  '],
      ['            ', ' {E}   .--.  ', '   ( @ )  ', '   ~~~~~~  ', '   ~~~~~~~ '],
      ['            ', ' {E}   .--.  ', '   ( · )  ', '   ~~~~~~  ', '   ~~~~~~  '],
    ],
    snarky: [
      ['            ', ' {E}   .--.~ ', '  \\ ( @ )~ ', '   \\_`--´~ ', '  ~~~~~~~~ '],
      ['            ', ' {E}   .--.~ ', '  \\ ( @ )~ ', '   \\_`--´  ', '  ~~~~~~~~ '],
      ['            ', ' {E}   .--.~ ', '  \\ ( @ )~ ', '   \\_`--´~ ', '  ~~~~~~~~ '],
    ],
  },
  [ghost]: {
    happy: [
      ['            ', '   .----. ~ ', '  / {E}  {E} \\~ ', '  |      |~ ', '  ~`~``~`~~ '],
      ['            ', '   .----. ~ ', '  / {E}  {E} \\~ ', '  |      |~ ', '  ~`~``~`~~ '],
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
    ],
    proud: [
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
      ['   ~    ~   ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
    ],
    curious: [
      ['            ', '   .----.   ', '  / {E}  {E} \\/ ', '  |      |  ', '  ~`~``~`~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\/ ', '  |      |  ', '  ~`~``~`~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\/ ', '  |      |  ', '  ~`~``~`~  '],
    ],
    pensive: [
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  `~``~`~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ~`~``~`~  '],
    ],
    tired: [
      ['            ', '   .----.   ', '  / ·  · \\  ', '  |      |  ', '  ``~~``~~  '],
      ['            ', '   .----.   ', '  / {E}  {E} \\  ', '  |      |  ', '  ``~~``~~~ '],
      ['            ', '   .----.   ', '  / ·  · \\  ', '  |      |  ', '  ``~~``~~  '],
    ],
    snarky: [
      ['            ', '   .----. ~ ', '  / {E}  {E} \\~ ', '  |      |~ ', '  `~``~`~`~ '],
      ['            ', '   .----. ~ ', '  / {E}  {E} \\~ ', '  |      |  ', '  `~``~`~`~ '],
      ['            ', '   .----. ~ ', '  / {E}  {E} \\~ ', '  |      |~ ', '  `~``~`~`~ '],
    ],
  },
  [axolotl]: {
    happy: [
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )~ ', '  (_/  \\_)~ '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )~ ', '  (_/  \\_)~ '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
    ],
    proud: [
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
      ['  ~      ~  ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
    ],
    curious: [
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )/ ', '  (_/  \\_)  '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )/ ', '  (_/  \\_)  '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )/ ', '  (_/  \\_)  '],
    ],
    pensive: [
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  (  --  )  ', '  (_/  \\_)  '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)  '],
    ],
    tired: [
      ['            ', '~}(______){~', '~}( · .. · ){~', '  ( .--. )  ', '  ~_/  \\_~  '],
      ['            ', '~}(______){~', '~}({E} .. {E}){~', '  ( .--. )  ', '  ~_/  \\_~ '],
      ['            ', '~}(______){~', '~}( · .. · ){~', '  ( .--. )  ', '  ~_/  \\_~  '],
    ],
    snarky: [
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )~ ', '  (_/  \\_)~ '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )  ', '  (_/  \\_)~ '],
      ['            ', '}~(______)~{', '}~({E} .. {E})~{', '  ( .--. )~ ', '  (_/  \\_)~ '],
    ],
  },
  [capybara]: {
    happy: [
      ['            ', '  n______n~', ' ( {E}    {E} )~', ' (   oo   )~', "  `------´~ "],
      ['            ', '  n______n~', ' ( {E}    {E} )~', ' (   oo   )~', "  `------´~ "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
    ],
    proud: [
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
      ['    ~  ~    ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
    ],
    curious: [
      ['            ', '  n______n ', ' ( {E}    {E} )/ ', ' (   oo   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} )/ ', ' (   oo   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} )/ ', ' (   oo   ) ', "  `------´  "],
    ],
    pensive: [
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   Oo   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´  "],
    ],
    tired: [
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   ··   ) ', "  `------´  "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   oo   ) ', "  `------´~ "],
      ['            ', '  n______n ', ' ( {E}    {E} ) ', ' (   ··   ) ', "  `------´  "],
    ],
    snarky: [
      ['            ', '  n______n~', ' ( {E}    {E} )~', ' (   oo   )~', "  `------´~ "],
      ['            ', '  n______n~', ' ( {E}    {E} )~', ' (   oo   ) ', "  `------´~ "],
      ['            ', '  n______n~', ' ( {E}    {E} )~', ' (   oo   )~', "  `------´~ "],
    ],
  },
  [cactus]: {
    happy: [
      ['            ', ' n  ____  n~', ' | |{E}  {E}| |~', ' |_|    |_|~', '   |    |~  '],
      ['            ', ' n  ____  n~', ' | |{E}  {E}| |~', ' |_|    |_|~', '   |    |~  '],
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
    ],
    proud: [
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
      [' n        n ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
    ],
    curious: [
      ['            ', ' n  ____  n/', ' | |{E}  {E}| |/', ' |_|    |_|/', '   |    |/  '],
      ['            ', ' n  ____  n/', ' | |{E}  {E}| |/', ' |_|    |_|/', '   |    |/  '],
      ['            ', ' n  ____  n/', ' | |{E}  {E}| |/', ' |_|    |_|/', '   |    |/  '],
    ],
    pensive: [
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
      ['            ', '    ____    ', ' n |{E}  {E}| n ', ' |_|    |_| ', '   |    |   '],
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |   '],
    ],
    tired: [
      ['            ', ' n  ____  n ', ' | | ·  · | | ', ' |_|    |_| ', '   |    |   '],
      ['            ', ' n  ____  n ', ' | |{E}  {E}| | ', ' |_|    |_| ', '   |    |~  '],
      ['            ', ' n  ____  n ', ' | | ·  · | | ', ' |_|    |_| ', '   |    |   '],
    ],
    snarky: [
      ['            ', ' n  ____  n~', ' | |{E}  {E}| |~', ' |_|    |_|~', '   |    |~  '],
      ['            ', ' n  ____  n~', ' | |{E}  {E}| |~', ' |_|    |_| ', '   |    |~  '],
      ['            ', ' n  ____  n~', ' | |{E}  {E}| |~', ' |_|    |_|~', '   |    |~  '],
    ],
  },
  [robot]: {
    happy: [
      ['            ', '   .[||]. ~ ', '  [ {E}  {E} ]~ ', '  [ ==== ]~ ', "  `------´~ "],
      ['            ', '   .[||]. ~ ', '  [ {E}  {E} ]~ ', '  [ ==== ]~ ', "  `------´~ "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
    ],
    proud: [
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
      ['     *      ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
    ],
    curious: [
      ['            ', '   .[||].   ', '  [ {E}  {E} ]/ ', '  [ ==== ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]/ ', '  [ ==== ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]/ ', '  [ ==== ]  ', "  `------´  "],
    ],
    pensive: [
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ -==- ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ ==== ]  ', "  `------´  "],
    ],
    tired: [
      ['            ', '   .[||].   ', '  [ ·  · ]  ', '  [ -==- ]  ', "  `------´  "],
      ['            ', '   .[||].   ', '  [ {E}  {E} ]  ', '  [ -==- ]  ', "  `------´~ "],
      ['            ', '   .[||].   ', '  [ ·  · ]  ', '  [ -==- ]  ', "  `------´  "],
    ],
    snarky: [
      ['            ', '   .[||]. ~ ', '  [ {E}  {E} ]~ ', '  [ ==== ]~ ', "  `------´~ "],
      ['            ', '   .[||]. ~ ', '  [ {E}  {E} ]~ ', '  [ ==== ]  ', "  `------´~ "],
      ['            ', '   .[||]. ~ ', '  [ {E}  {E} ]~ ', '  [ ==== ]~ ', "  `------´~ "],
    ],
  },
  [rabbit]: {
    happy: [
      ['            ', '   (\\__/) ~ ', '  ( {E}  {E} )~ ', ' =(  ..  )=~', '  (")__(")~ '],
      ['            ', '   (\\__/) ~ ', '  ( {E}  {E} )~ ', ' =(  ..  )=~', '  (")__(")~ '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
    ],
    proud: [
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
      ['   ~    ~   ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
    ],
    curious: [
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )/ ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )/ ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )/ ', ' =(  ..  )= ', '  (")__(")  '],
    ],
    pensive: [
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (|__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")  '],
    ],
    tired: [
      ['            ', '   (\\__/)   ', '  ( ·  · )  ', ' =(  ..  )= ', '  (")__(")  '],
      ['            ', '   (\\__/)   ', '  ( {E}  {E} )  ', ' =(  ..  )= ', '  (")__(")~ '],
      ['            ', '   (\\__/)   ', '  ( ·  · )  ', ' =(  ..  )= ', '  (")__(")  '],
    ],
    snarky: [
      ['            ', '   (\\__/)~  ', '  ( {E}  {E} )~ ', ' =(  ..  )=~', '  (")__(")~ '],
      ['            ', '   (\\__/)~  ', '  ( {E}  {E} )~ ', ' =(  ..  )= ', '  (")__(")~ '],
      ['            ', '   (\\__/)~  ', '  ( {E}  {E} )~ ', ' =(  ..  )=~', '  (")__(")~ '],
    ],
  },
  [mushroom]: {
    happy: [
      ['            ', ' .-o-OO-o-.~', '(__________)', '   |{E}  {E}|~  ', '   |____|~  '],
      ['            ', ' .-o-OO-o-.~', '(__________)', '   |{E}  {E}|~  ', '   |____|~  '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
    ],
    proud: [
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
      ['   . o  .   ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
    ],
    curious: [
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|/  ', '   |____|/  '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|/  ', '   |____|/  '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|/  ', '   |____|/  '],
    ],
    pensive: [
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
      ['            ', ' .-O-oo-O-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|   '],
    ],
    tired: [
      ['            ', ' .-o-OO-o-. ', '(__________)', '   | ·  · |   ', '   |____|   '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   |{E}  {E}|   ', '   |____|~  '],
      ['            ', ' .-o-OO-o-. ', '(__________)', '   | ·  · |   ', '   |____|   '],
    ],
    snarky: [
      ['            ', ' .-o-OO-o-.~', '(__________)', '   |{E}  {E}|~  ', '   |____|~  '],
      ['            ', ' .-o-OO-o-.~', '(__________)', '   |{E}  {E}|   ', '   |____|~  '],
      ['            ', ' .-o-OO-o-.~', '(__________)', '   |{E}  {E}|~  ', '   |____|~  '],
    ],
  },
  [chonk]: {
    happy: [
      ['            ', '  /\\    /\\~ ', ' ( {E}    {E} )~ ', ' (   ..   )~', "  `------´~ "],
      ['            ', '  /\\    /\\~ ', ' ( {E}    {E} )~ ', ' (   ..   )~', "  `------´~ "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
    ],
    proud: [
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
      ['  /|    |\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
    ],
    curious: [
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} )/ ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} )/ ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} )/ ', ' (   ..   ) ', "  `------´  "],
    ],
    pensive: [
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /|  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´  "],
    ],
    tired: [
      ['            ', '  /\\    /\\  ', ' ( ·    · ) ', ' (   ..   ) ', "  `------´  "],
      ['            ', '  /\\    /\\  ', ' ( {E}    {E} ) ', ' (   ..   ) ', "  `------´~ "],
      ['            ', '  /\\    /\\  ', ' ( ·    · ) ', ' (   ..   ) ', "  `------´  "],
    ],
    snarky: [
      ['            ', '  /\\    /\\~ ', ' ( {E}    {E} )~ ', ' (   ..   )~', "  `------´~ "],
      ['            ', '  /\\    /\\~ ', ' ( {E}    {E} )~ ', ' (   ..   ) ', "  `------´~ "],
      ['            ', '  /\\    /\\~ ', ' ( {E}    {E} )~ ', ' (   ..   )~', "  `------´~ "],
    ],
  },
}

const HAT_LINES: Record<Hat, string> = {
  none: '',
  crown: '   \\^^^/    ',
  tophat: '   [___]    ',
  propeller: '    -+-     ',
  halo: '   (   )    ',
  wizard: '    /^\\     ',
  beanie: '   (___)    ',
  tinyduck: '    ,>      ',
}

export function renderSprite(bones: CompanionBones, frame = 0): string[] {
  const frames = BODIES[bones.species]
  const body = frames[frame % frames.length]!.map(line =>
    line.replaceAll('{E}', bones.eye),
  )
  const lines = [...body]
  // Only replace with hat if line 0 is empty (some fidget frames use it for smoke etc)
  if (bones.hat !== 'none' && !lines[0]!.trim()) {
    lines[0] = HAT_LINES[bones.hat]
  }
  // Drop blank hat slot — wastes a row in the Card and ambient sprite when
  // there's no hat and the frame isn't using it for smoke/antenna/etc.
  // Only safe when ALL frames have blank line 0; otherwise heights oscillate.
  if (!lines[0]!.trim() && frames.every(f => !f[0]!.trim())) lines.shift()
  return lines
}

export function spriteFrameCount(species: Species): number {
  return BODIES[species].length
}

export function renderFace(bones: CompanionBones): string {
  const eye: Eye = bones.eye
  switch (bones.species) {
    case duck:
    case goose:
      return `(${eye}>`
    case blob:
      return `(${eye}${eye})`
    case cat:
      return `=${eye}ω${eye}=`
    case dragon:
      return `<${eye}~${eye}>`
    case octopus:
      return `~(${eye}${eye})~`
    case owl:
      return `(${eye})(${eye})`
    case penguin:
      return `(${eye}>)`
    case turtle:
      return `[${eye}_${eye}]`
    case snail:
      return `${eye}(@)`
    case ghost:
      return `/${eye}${eye}\\`
    case axolotl:
      return `}${eye}.${eye}{`
    case capybara:
      return `(${eye}oo${eye})`
    case cactus:
      return `|${eye}  ${eye}|`
    case robot:
      return `[${eye}${eye}]`
    case rabbit:
      return `(${eye}..${eye})`
    case mushroom:
      return `|${eye}  ${eye}|`
    case chonk:
      return `(${eye}.${eye})`
  }
}

export { EMOTION_BODIES }
